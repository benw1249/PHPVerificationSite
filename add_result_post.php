...
if(isset($_SESSION['user_data'])){
    ...
    if($qr_result){
        ...
        $permit_Array=$_REQUEST['permit_id'];
        $subject_id_array=$_REQUEST['id'];
        foreach($permit_Array as $key=>$permit){
            $permit_qr=mysqli_query($con,"INSERT into result_data (result_id,subject_id,permitid) values ('".$last_id."','".$subject_id_array[$key]."','".$permit."')");
        }
